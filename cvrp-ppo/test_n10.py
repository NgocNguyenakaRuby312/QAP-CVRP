#!/usr/bin/env python
"""test_n10.py — Evaluate trained QAP-DRL on CVRP-10 test set."""
...
CAPACITY        = 20
LKH3_REF        = 4.84
TEST_PATH       = os.path.join(SCRIPT_DIR, "datasets", "test_n10.pkl")
MODEL_PATH      = os.path.join(SCRIPT_DIR, "outputs", "n10", "best_model.pt")
...
def plot_route_map(coords_np, actions_list, graph_size, tour_length,
    ...
    for i in range(1, graph_size + 1):
        ax.plot(coords_np[i, 0], coords_np[i, 1], "o",
                color="#4a90d9", markersize=8, zorder=4)
...
def main():
    ...
    if not os.path.exists(MODEL_PATH):
        ...
        print("       Train first with: python train_n10.py")
        sys.exit(1)
